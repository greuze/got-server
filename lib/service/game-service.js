...
module.exports.createGame = function(args, done) {
...
module.exports.getGame = function(args, done) {
...
// args.gameId: Id of the game
// args.playerId: Id of the player that did the request
module.exports.muster = function(args, done) {
...
// args.gameId: Id of the game
// args.playerId: Id of the player that did the request
module.exports.getWildlingsTop = function(args, done) {
    var game = storage.getRunningGame(args.gameId);
    if (!game) {
        return done(errors.NOT_FOUND);
    }

    if (!validator.canGetWildlingsTop(game, args.playerId)) {
        return done(errors.WRONG_PARAMETERS);
    }

    var wildlingsTop = action.getWildlingsTop(game);

    return done(null, wildlingsTop);
};

// args.gameId: Id of the game
// args.playerId: Id of the player that did the request
// args.skip: Tells if top of deck should be skipped or not
module.exports.skipWildlingsTop = function(args, done) {
    var game = storage.getRunningGame(args.gameId);
    if (!game) {
        return done(errors.NOT_FOUND);
    }

    if (!validator.canSkipWildlingsTop(game, args.playerId)) {
        return done(errors.WRONG_PARAMETERS);
    }

    action.skipWildlingsTop(game, args.skip);
...
module.exports.replaceOrder = function(args, done) {
    var game = storage.getRunningGame(args.gameId);
    if (!game) {
        return done(errors.NOT_FOUND);
    }
...
// args.gameId: Id of the game
// args.playerId: Id of the player that did the request
module.exports.bet = function(args, done) {
